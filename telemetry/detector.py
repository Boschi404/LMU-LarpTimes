...
class LapBoundaryDetector:
    def __init__(self, db_path: str = database.DEFAULT_DB_PATH, fallback_pit_loss: float = 30.0, on_session_complete=None, on_race_started=None, on_qualifying_started=None):
        ...
        # Optional callback: on_qualifying_started(session_uuid, car, track)
        # Fired when a new QUALIFYING session starts.
        self.on_qualifying_started = on_qualifying_started
    ...
    def _create_session(self, frame: TelemetryFrame) -> None:
        ...
        # Fire appropriate callback based on session type
        if self.session_type == "RACE" and self.on_race_started:
            try:
